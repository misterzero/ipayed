...
export class BankAccountTypeComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.bankAccountTypeService.query({
            ...
            (res: HttpResponse<BankAccountType[]>) => this.onSuccess(res.body, res.headers),
    ...
    registerChangeInBankAccountTypes() {
        this.eventSubscriber = this.eventManager.subscribe('bankAccountTypeListModification', (response) => this.reset());
    }
    ...
    private onSuccess(data, headers) {
        ...
        for (let i = 0; i < data.length; i++) {
            this.bankAccountTypes.push(data[i]);
        }
    }
