...
import { BankAccountType, BankAccountTypeService } from '../bank-account-type';
...
export class BankAccountDialogComponent implements OnInit {
    ...
    ngOnInit() {
        this.isSaving = false;
        this.bankAccountTypeService.query()
            .subscribe((res: HttpResponse<BankAccountType[]>) => { this.bankaccounttypes = res.body; }, (res: HttpErrorResponse) => this.onError(res.message));
    ...
    trackBankAccountTypeById(index: number, item: BankAccountType) {
        return item.id;
    }
